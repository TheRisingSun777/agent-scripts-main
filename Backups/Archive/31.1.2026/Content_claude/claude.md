# Claude Working Agreement (Repo Pointer)
...
Then read, in order:
1) ./AGENTS.md (repo-local contract: entrypoints + gates + scope)
2) ./docs/00_START_HERE.md (doc ladder + minimal reading set)
3) ./.claude/GOALS.md (execution plan + verification gates)
4) ./.claude/TASKS.md (or tasks.md), ./.claude/ISSUES.md, ./.claude/DECISIONS.md, ./.claude/PROGRESS.md

Rules precedence (highest → lowest):
1) Control plane AGENTS.MD
2) Repo AGENTS.md
3) .claude/* (state + evidence; cannot override guardrails)
